...
##metadata for BAMs from GDC data portal
...
def get_overlap(tumor):
    ...
    try:
        bed_df = pd.read_csv(cwd / 'files' / 'coverage_beds' / file, names=['Chromosome', 'Start', 'End'], low_memory=False, sep='\t')
    except:
        return None
...
tcga_sample_table = pd.read_csv(cwd / 'files' / 'TCGA-CDR-SupplementalTableS1.tsv', sep='\t').iloc[:, 1:]
...
with open(cwd / 'files' / 'tcga_public_maf.pkl', 'wb') as f:
    pickle.dump(final_df, f)

with open(cwd / 'files' / 'tcga_public_sample_table.pkl', 'wb') as f:
