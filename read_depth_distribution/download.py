...
##your token
with open(cwd / 'files' / 'bams' / 'gdc-user-token.2021-12-13T20_06_44.138Z.txt') as f:
    token = f.read()
...
def load_url(file, regions, name):
    ...
    with open(cwd / 'files' / 'bams' / 'samples' / (name + '.bam'), 'wb') as f:
        f.write(r.content)
    return len(r.content)

to_download = sorted(list(samples_to_file.keys()))

##sometimes there are errors in the download
while True:
